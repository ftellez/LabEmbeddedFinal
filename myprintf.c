...
uint8_t receiveFromRasp() {
    if(SERCOM2->USART.INTFLAG.bit.RXC){
	uint8_t data = SERCOM2->USART.DATA.reg;
	myputchar(data);
	return data;
    }
    return 0;
...
static void sendUART2char(char s)
{
  while (!(SERCOM2->USART.INTFLAG.reg & SERCOM_USART_INTFLAG_DRE));
  SERCOM2->USART.DATA.reg = s;
}

/* "print.h" uses this function for is's printf implementation */
static void myputchar2(char c)
{
  sendUART2char(c);
}
...
//{
    ...
    //{ 
	  //if(*s=='\n')
		  //sendUART2char('\r');
	   //
	   //sendUART2char(*s); /*Send char to the UART1*/	   
	   //s++; /* Next char */
    //}
    //return 0;
//}
